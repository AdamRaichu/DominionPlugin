...
public class FactionQueryCommand extends AbstractPlayerCommand {
  ...
  protected void execute(@Nonnull CommandContext commandContext, @Nonnull Store<EntityStore> store,
    ...
    FactionComponent faction = store.getComponent(ref, DominionPlugin.get().getFactionComponentType());
    ...
    switch (faction.getFaction()) {
      case Faction.HRE:
        player.sendMessage(Message.raw("You are a member of the Holy Roman Empire!"));
        break;

      case Faction.OE:
        player.sendMessage(Message.raw("You are a member of the Ottoman Empire!"));
        break;

      default:
        LOGGER.atWarning().log("Somehow player " + playerRef.getUsername() + "got to this point.");
        break;
    }
  }
}
