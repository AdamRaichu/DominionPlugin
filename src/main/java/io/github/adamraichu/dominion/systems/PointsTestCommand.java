...
public class PointsTestCommand extends AbstractPlayerCommand {
  ...
  protected void execute(@Nonnull CommandContext commandContext, @Nonnull Store<EntityStore> store,
    ...
    PointsComponent points = store.getComponent(ref, DominionPlugin.get().getPointsComponentType());
    if (Objects.isNull(points)) {
      LOGGER.atInfo().log("Player had no points component, adding...");
      points = new PointsComponent();
      store.addComponent(ref, DominionPlugin.get().getPointsComponentType(), points);
    }
    ...
    player.sendMessage(Message.raw("You have been given 2 points!").color("Green").bold(true));
  }
}
