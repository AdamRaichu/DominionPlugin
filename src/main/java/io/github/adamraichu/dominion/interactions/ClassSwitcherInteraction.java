...
public class ClassSwitcherInteraction extends SimpleInstantInteraction {
  ...
  protected void firstRun(
    ...
    world.execute(() -> {
      ...
      FighterClassComponent fighterClass = world
          .getEntityStore()
          .getStore()
          .getComponent(ref, DominionPlugin.get().getFighterClassComponentType());

      // Cycle to the next class. In the future we may add UI, but for now this is
      // fine.
      int currentIndex = FighterClass.values.indexOf(
