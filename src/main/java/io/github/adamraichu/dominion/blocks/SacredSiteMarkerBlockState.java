...
public class SacredSiteMarkerBlockState
    extends BlockState
    implements TickableBlockState {

  public static final Codec<SacredSiteMarkerBlockState> CODEC = BuilderCodec.builder(
      SacredSiteMarkerBlockState.class,
      SacredSiteMarkerBlockState::new,
      ...
      .append(
          new KeyedCodec<>(
              "SpawnPoints",
              SacredSiteMarkerBlockState.SpawnPoints.CODEC),
          (o, v) -> {
            o.spawnPoints.clear();
  ...
  private Map<Faction, Vector3d> spawnPoints = new java.util.EnumMap<>(
      Faction.class);
  ...
  public void setSpawnPoint(Faction faction, Vector3d point) {
    LOGGER.atInfo().log(
        "Setting spawn point for faction " + faction + " to " + point.toString());
  ...
  public void tick(
    ...
    for (PlayerRef playerRef : playersInZone) {
      FactionComponent faction = Universe.get()
          .getWorld(playerRef.getWorldUuid())
          .getEntityStore()
          .getStore()
          .getComponent(
  ...
  public void startGame(PlayerRef[] players, Store<EntityStore> store) {
    LOGGER.atInfo().log(
        "Starting game at " + this.getBlockPosition().toString());
    ...
    for (PlayerRef playerRef : trackedPlayers_) {
      ...
      playerRef.sendMessage(
          Message.raw("You are now on faction " + faction.getFaction()));
      playerRef
          .getReference()
          .getStore()
          .addComponent(
              ...
              Teleport.createForPlayer(
                  spawnPoints.get(faction.getFaction()),
                  new Vector3f((float) -3.14, 0, 0)));
      playerIdx++;
    }
  ...
  private List<PlayerRef> getPlayersWithinZone() {
    ...
    Store<EntityStore> store = Universe.get()
        .getDefaultWorld()
        .getEntityStore()
        .getStore();
  ...
  public static class SpawnPoints extends StateData {
    ...
    public static final Codec<SpawnPoints> CODEC = BuilderCodec.builder(
        SpawnPoints.class,
        SpawnPoints::new,
        StateData.DEFAULT_CODEC)
        .append(
            new KeyedCodec<>("HreX", Codec.FLOAT),
            (o, v) -> o.hreX = v,
            o -> o.hreX)
        .add()
        .append(
            new KeyedCodec<>("HreY", Codec.FLOAT),
            (o, v) -> o.hreY = v,
            o -> o.hreY)
        .add()
        .append(
            new KeyedCodec<>("HreZ", Codec.FLOAT),
            (o, v) -> o.hreZ = v,
            o -> o.hreZ)
        .add()
        .append(
            new KeyedCodec<>("OeX", Codec.FLOAT),
            (o, v) -> o.oeX = v,
            o -> o.oeX)
        .add()
        .append(
            new KeyedCodec<>("OeY", Codec.FLOAT),
            (o, v) -> o.oeY = v,
            o -> o.oeY)
        .add()
        .append(
            new KeyedCodec<>("OeZ", Codec.FLOAT),
            (o, v) -> o.oeZ = v,
            o -> o.oeZ)
        .add()
        .build();
  ...
  public static class Data extends StateData {
    ...
    public static final BuilderCodec<SacredSiteMarkerBlockState.Data> CODEC = BuilderCodec.builder(
        SacredSiteMarkerBlockState.Data.class,
        SacredSiteMarkerBlockState.Data::new,
        StateData.DEFAULT_CODEC).build();
  }
